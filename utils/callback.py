...
class CustomCallback(BaseCallback):
    ...
    def _on_rollout_start(self) -> None:
        ...
        """
        self.rollout += 1
        if self.rollout % self.model_save_freq == 0:
            save_name = os.path.join(self.model_save_path, str(self.num_timesteps).zfill(10))
            self.save_model(save_name)
            print(f'Saving model {save_name}')
        if self.rollout % self.model_eval_freq == 0:
            pass
            # mean_reward, std_reward = evaluate_policy(self.model, self.eval_env, n_eval_episodes=250, deterministic=True)
            # print(f'Validation: {mean_reward}/{std_reward}')
            # self.logger.record('Validation Reward', mean_reward)
        if self.rollout % self.model_plot_freq == 1:
    ...
    def _on_rollout_end(self) -> None:
        ...
        if self.plotting:
            self.plot_rollout += 1
            if self.plot_rollout == 16:
                prices = self.trajectory_data[:self.plot_time, :5]
                ...
                plt.plot(prices[:, 4][:-1])
                plt.ylabel("USD/XRP") 
                plt.savefig(os.path.join(self.model_plot_path, f'prices_{str(self.num_timesteps).zfill(10)}.png'))

                accounts = self.trajectory_data[:self.plot_time, 5:]
                plt.figure(figsize=(8, 14))
                plt.subplot(711)
                plt.plot(accounts[:, 6][:-1])
                plt.ylabel("Portfolio (USD)")                 
                plt.subplot(712)
                ...
                plt.ylabel("XRP (USD)")              
                plt.savefig(os.path.join(self.model_plot_path, f'account_{str(self.num_timesteps).zfill(10)}.png'))
    ...
    def add_plot_point(self):
        ...
        self.trajectory_data[self.plot_time, :5] = price_data[:, 3]
