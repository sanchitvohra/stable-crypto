...
class CryptoEnvDict(gym.Env):
    def __init__(self, data, balance, max_trade, trading_fee, history_len, mean, std, ep_len, validate):
        ...
        self.observation_space = spaces.Dict({'price' : spaces.Box(low=-1, high=1, shape=state['price'].shape, dtype=np.float32),
                                              'account': spaces.Box(low=-10, high=10, shape=state['account'].shape, dtype=np.float32)})
    ...
    def step(self, actions):
        ...
        self.n_step += 1
        return self.get_state(False), self.portfolio - old_portfolio, self.n_step == self.cu_len, {}
